...
template <typename T>
int computePinPosCudaLauncher(const T* x, const T* y, const T* z, const T* pin_offset_x,
                              const T* pin_offset_y, const T* pin_offset_z, 
...
int computePinPosGradCudaLauncher(
    ...
    const T* pin_offset_x, const T* pin_offset_y, const T* pin_offset_z,
    const long* pin2node_map, const int* flat_node2pin_map, 
    const int* flat_node2pin_start_map,
    int num_nodes, int num_pins, T* grad, T* grad_y, T* grad_z);

at::Tensor pin_pos_forward(at::Tensor pos, at::Tensor pin_offset_x,
                           at::Tensor pin_offset_y, at::Tensor pin_offset_z,
                           at::Tensor pin2node_map, at::Tensor flat_node2pin_map,
                           at::Tensor flat_node2pin_start_map) {
  CHECK_FLAT_CUDA(pos);
  CHECK_EVEN(pos);
  CHECK_CONTIGUOUS(pos);

  auto out = at::zeros(pin_offset_x.numel() * 3, pos.options());  //3D modify
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computePinPosCudaLauncher", [&] {
        computePinPosCudaLauncher<scalar_t>(
            ...
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_nodes * 2, // 3D modify z position
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_x, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_y, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_z, scalar_t),
            ...
            DREAMPLACE_TENSOR_DATA_PTR(out, scalar_t) + num_pins*2); // 3D modify z position
      });
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computePinPosGradCudaLauncher", [&] {
        computePinPosGradCudaLauncher<scalar_t>(
            ...
            DREAMPLACE_TENSOR_DATA_PTR(grad_out, scalar_t) + num_pins*2,  //3D modify  z
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_nodes,
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_nodes *2, //3D modify
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_x, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_y, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_z, scalar_t),  //3D modify
            DREAMPLACE_TENSOR_DATA_PTR(pin2node_map, long),
            DREAMPLACE_TENSOR_DATA_PTR(flat_node2pin_map, int),
            ...
            DREAMPLACE_TENSOR_DATA_PTR(out, scalar_t) + num_nodes * 2);
      });
