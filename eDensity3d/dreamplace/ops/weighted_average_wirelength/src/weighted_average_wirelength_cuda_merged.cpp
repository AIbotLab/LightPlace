...
void integrateNetWeightsCudaLauncher(const int* pin2net_map,
                                     ...
                                     T* grad_y_tensor, /*T* grad_z_tensor,*/ int num_pins);
...
std::vector<at::Tensor> weighted_average_wirelength_forward(
  ...
  int num_pins = pos.numel() / 3;  // 3D modify  这里的pos为经历过pin_pos_op转换后的每个引脚的坐标

  // x, y interleave
  at::Tensor partial_wl = at::zeros({num_nets, 3}, pos.options());  //3D modify 
...
at::Tensor weighted_average_wirelength_backward(
  ...
  // int num_nets = netpin_start.numel() - 1;
  int num_pins = pos.numel() / 2;  // 3D modify
