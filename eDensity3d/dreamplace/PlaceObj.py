...
class PreconditionOp:
    ...
    def __call__(self, grad, density_weight, update_mask=None):
        ...
        with torch.no_grad():
            ...
            # # 3D modify
            # grad[self.placedb.num_nodes * 2 : self.placedb.num_nodes * 3].div_(precond)
...
class PlaceObj(nn.Module):
    ...
    def __init__(self, density_weight, params, placedb, data_collections,
        ...
        if len(placedb.regions) > 0:
            ...
            self.density_weight = torch.tensor(
                [density_weight],  # density_weight is 0.0
                dtype=self.data_collections.pos[0].dtype,
                device=self.data_collections.pos[0].device)
        ...
        num_bins_z = global_place_params["num_bins_z"] if "num_bins_z" in global_place_params and global_place_params["num_bins_z"] > 1 else placedb.num_bins_z
        name = "Global placement: %dx%dx%d bins by default" % (num_bins_x, num_bins_y,num_bins_z)
        logging.info(name)
        ...
        self.bin_size_z = (placedb.zh - placedb.zl) / num_bins_z
        # 3D modify  这里暂时不对z轴 bin大小做修改
        ...
        name = "%dx%dx%d bins" % (num_bins_x, num_bins_y, num_bins_z)
        self.name = name
    ...
    def obj_fn(self, pos):
        ...
        """
        self.wirelength = self.op_collections.wirelength_op(pos) # 3D modify  这是近似线长  梯度也已经算出来了
        if len(self.placedb.regions) > 0:
            self.density = self.op_collections.fence_region_density_merged_op(pos)
        else:
            self.density = self.op_collections.density_op(pos) # 3D modify  存在bug
        ...
        if len(self.placedb.regions) > 0:
            ...
            # result = self.density
            # print("self.density_factor:",self.density_factor)
            # print("self.wirelength:",self.wirelength)
    ...
    def obj_and_grad_fn(self, pos):
        ...
        # obj.backward(retain_graph=True)  # 调用 electric_potential.ElectricPotentialFunction 的 backward
        obj.backward()
        self.op_collections.precondition_op(pos.grad, self.density_weight, self.update_mask)

        # dot = make_dot(obj, params=dict(self.named_parameters()))
        # dot.format = 'png'  # 设置保存格式为 PNG
        # dot.render('backpropagation_graph')  # 保存图像为 backpropagation_graph.png     
        # print("backpropagation_graph success")
    ...
    def build_weighted_average_wl(self, params, placedb, data_collections,
        ...
        wirelength_for_pin_op = weighted_average_wirelength.WeightedAverageWirelength(
            ...
            gamma=self.gamma,
            algorithm='merged',
            zAlpha = placedb.zAlpha) # 3D modify

        # wirelength for position
        def build_wirelength_op(pos):  # 3D modify
    ...
    def build_electric_overflow(self, params, placedb, data_collections,
        ...
        @parem num_bins_z number of bins in z depth
        """
        bin_size_x = (placedb.xh - placedb.xl) / num_bins_x
        bin_size_y = (placedb.yh - placedb.yl) / num_bins_y
        bin_size_z = (placedb.zh - placedb.zl) / num_bins_z # 3D modify

        return electric_overflow.ElectricOverflow(
            node_size_x=data_collections.node_size_x,
            node_size_y=data_collections.node_size_y,
            node_size_z=data_collections.node_size_z,  # 3D modify
            bin_center_x=data_collections.bin_center_x_padded(placedb, 0, num_bins_x),
            bin_center_y=data_collections.bin_center_y_padded(placedb, 0, num_bins_y),
            bin_center_z=data_collections.bin_center_z_padded(placedb, 0, num_bins_z),
            target_density=data_collections.target_density,
            xl=placedb.xl,
            yl=placedb.yl,
            zl=placedb.zl,
    ...
    def build_electric_potential(self, params, placedb, data_collections,
                                 num_bins_x, num_bins_y, num_bins_z, name, region_id=None, fence_regions=None):
        """
        ...
        max_num_bins_z = np.ceil(
            (np.amax(placedb.node_size_z[0:placedb.num_movable_nodes]) +
             2 * bin_size_z) / bin_size_z)
       
        max_num_bins = max( max(int(max_num_bins_x), int(max_num_bins_y)), int(max_num_bins_z))
        logging.info(
            "%s #bins %dx%dx%d, bin sizes %gx%gx%g, max_num_bins = %d, padding = %d"
            % (name, num_bins_x, num_bins_y, num_bins_z,
               bin_size_x, bin_size_y, bin_size_z, max_num_bins, 0))
        ...
        if num_bins_z < max_num_bins:
            logging.warning("num_bins_z (%d) < max_num_bins (%d)" %
                            (num_bins_z, max_num_bins))
        ...
        return electric_potential.ElectricPotential(
            node_size_x=data_collections.node_size_x,
            node_size_y=data_collections.node_size_y,
            node_size_z=data_collections.node_size_z,
            bin_center_x=data_collections.bin_center_x_padded(placedb, 0, num_bins_x),
            bin_center_y=data_collections.bin_center_y_padded(placedb, 0, num_bins_y),
            bin_center_z=data_collections.bin_center_z_padded(placedb, 0, num_bins_z),
    ...
    def build_update_density_weight(self, params, placedb, algo="overflow"):
        ...
        ref_hpwl = params.RePlAce_ref_hpwl      #350000
        LOWER_PCOF = params.RePlAce_LOWER_PCOF  #0.95
        UPPER_PCOF = params.RePlAce_UPPER_PCOF  #1.05
        UPPER_PCOF = 1.005
        #使用FFTPL论文中的参数
        LOWER_PCOF = 0.75
        UPPER_PCOF = 1.1
        ...
        def update_density_weight_op_hpwl(cur_metric, prev_metric, iteration):
            ...
            with torch.no_grad():
                ...
                # print("delta_hpwl:",delta_hpwl)
                # 这是RePlace的策略
                if delta_hpwl < 0:
                    mu = UPPER_PCOF * np.maximum(
                        np.power(0.9999, float(iteration)), 0.98)
                else:
                    mu = UPPER_PCOF * torch.pow(  # 对应FFTPL论文中的策略 uk = UPPER_PCOF**(-(w_k-w_ref)+1) 
                ...
                # print("self.density_weight:",self.density_weight)
    ...
    def base_gamma(self, params, placedb):
        ...
        @param placedb placement database
        """
        return params.gamma * (self.bin_size_x + self.bin_size_y + self.bin_size_z)
    ...
    def update_gamma(self, iteration, overflow, base_gamma):
        ...
        # coef = torch.pow(10, (overflow_avg - 0.1) * 4 / 1000 - 1)
        coef = torch.pow(10, (overflow_avg - 0.1) * 20 / 9 - 1)
        # 使用FFTPL中的系数变化  gamma = 8*w_b*10**((20/9)*(overflow-0.1)-1)
    ...
    def build_noise(self, params, placedb, data_collections):
        ...
        """
        node_size = torch.cat(
            [data_collections.node_size_x, data_collections.node_size_y, data_collections.node_size_z],
        ...
        def noise_op(pos, noise_ratio):
            with torch.no_grad():
                ...
                noise[placedb.num_nodes +
                      ...
                      placedb.num_filler_nodes].zero_()
                noise[2 * placedb.num_nodes +
                      placedb.num_movable_nodes:3 * placedb.num_nodes -
                      placedb.num_filler_nodes].zero_()  #z轴噪音
