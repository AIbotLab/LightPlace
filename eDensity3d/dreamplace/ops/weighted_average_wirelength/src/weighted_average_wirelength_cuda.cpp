...
std::vector<at::Tensor> weighted_average_wirelength_forward(
  ...
  at::Tensor exp_nxy = at::empty_like(pos);  //对应 a_i
  at::Tensor exp_xy_sum = at::zeros({3, num_nets}, pos.options());
  at::Tensor exp_nxy_sum = at::zeros({3, num_nets}, pos.options());
  at::Tensor xyexp_xy_sum = at::zeros({3, num_nets}, pos.options());
  at::Tensor xyexp_nxy_sum = at::zeros({3, num_nets}, pos.options());
  ...
  at::Tensor xy_max = at::full({3, num_nets}, std::numeric_limits<V>::min(),
                               at::CUDA(at::kInt));
  at::Tensor xy_min = at::full({3, num_nets}, std::numeric_limits<V>::max(),
                               at::CUDA(at::kInt));
