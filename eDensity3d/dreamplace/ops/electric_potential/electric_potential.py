...
import dreamplace.ops.dct.dct as dct1d
...
class ElectricPotentialFunction(Function):
    ...
    @staticmethod
    def idxst_pre(x):
        xx = x.reshape([-1,x.size(-1)])
        N = xx.size(-1)
        M = int(xx.numel() / N)
        xx_reorder = torch.zeros(M*N)
        xx = xx.reshape(-1)
        for i in range(M*N):
            ii = i % N
            xx_reorder[i] = xx[i + N - ii * 2] if ii else 0
        return xx_reorder.view(M, N)
    
    def idxst_post(x, origin_x):
        x = x*0.5
        N = x.size(-1)
        M = int(x.numel() / N)
        x = x.view(-1)
        for i in range(int(M*(N/2))):
            x[i*2+1] = -x[i*2+1]
        return x.view(origin_x.size())

    @staticmethod
    def forward(
        ctx,pos,node_size_x_clamped,node_size_y_clamped,node_size_z_clamped,
        ...
        bin_size_x,bin_size_y,bin_size_z,num_movable_nodes,num_filler_nodes,
        padding,padding_mask,  # same dimensions as density map, with padding regions to be 1
        ...
        exact_expkZ=None,  # exp(-j*pi*k/N)
        inv_wj2_plus_wk2_plus_wl2 = None,
        wj_by_wj2_plus_wk2_plus_wl2_half = None, 
        wk_by_wj2_plus_wk2_plus_wl2_half = None,
        wl_by_wj2_plus_wk2_plus_wl2_half = None,
        ...
        dct=None,idct=None,idxst=None,fast_mode=True): # fast mode will discard some computation
        ...
        density_map = ElectricDensityMapFunction.forward(
            ...
            offset_x, offset_y, offset_z, ratio,
            bin_center_x, bin_center_y, bin_center_z, initial_density_map,
            target_density, xl, yl, zl, xh, yh, zh, bin_size_x, bin_size_y, bin_size_z,
        ...
        ctx.node_size_z_clamped = node_size_z_clamped # 3D modify
        ctx.offset_x = offset_x
        ctx.offset_y = offset_y
        ctx.offset_z = offset_z
        ctx.ratio = ratio
        ctx.bin_center_x = bin_center_x
        ctx.bin_center_y = bin_center_y
        ctx.bin_center_z = bin_center_z
        ...
        M = num_bins_x
        N = num_bins_y
        Z = num_bins_z  # 3D modify

        # wj and wk wl
        if inv_wj2_plus_wk2_plus_wl2 is None:
            wj = torch.arange(M, dtype=pos.dtype, device=pos.device).mul(2 * np.pi / M).view([M, 1, 1])
            # scale wv because the aspect ratio of a bin may not be 1
            wk = torch.arange(N, dtype=pos.dtype, device=pos.device).mul(2 * np.pi / N).view([1, N, 1])

            wl = torch.arange(Z, dtype=pos.dtype, device=pos.device).mul(2 * np.pi / Z).view([1, 1, Z])

            wj2_plus_wk2_plus_wl2 = wj.pow(2) + wk.pow(2) + wl.pow(2)  # 矩阵会变成 M * N的矩阵，广播机制
            wj2_plus_wk2_plus_wl2[0, 0, 0] = 1.0  # avoid zero-division, it will be zeroed out
            inv_wj2_plus_wk2_plus_wl2 = 1.0 / wj2_plus_wk2_plus_wl2
            inv_wj2_plus_wk2_plus_wl2[0, 0, 0] = 0.0
            wj_by_wj2_plus_wk2_plus_wl2_half = wj.mul(inv_wj2_plus_wk2_plus_wl2).mul_(1. / 2)
            wk_by_wj2_plus_wk2_plus_wl2_half = wk.mul(inv_wj2_plus_wk2_plus_wl2).mul_(1. / 2)
            wl_by_wj2_plus_wk2_plus_wl2_half = wl.mul(inv_wj2_plus_wk2_plus_wl2).mul_(1. / 2)

        # desity_map 就是密度rou
        # compute auv
        density_map.mul_(1.0 / (ctx.bin_size_x * ctx.bin_size_y * ctx.bin_size_z))

        #auv = discrete_spectral_transform.dct2_2N(density_map, expk0=exact_expkM, expk1=exact_expkN)
        # auv = dct2.forward(density_map)
        """ #ajkl2与ajkl相同   torch.allclose(ajkl,ajkl2, rtol=1e-03, atol = 1e-05)
        ajkl2 =  dct.forward(
                    dct.forward(
                        dct.forward(
                            density_map
                        ).transpose(0,2).contiguous().transpose(1,2).contiguous()
                    ).transpose(0,2).contiguous().transpose(1,2).contiguous()
                ).transpose(0,2).contiguous().transpose(1,2).contiguous()
        #"""
        ajkl =  dct.forward(
        ...
        """ #与开源库验证过了，是一致的
        ajkl2 = torch_dct.dct_3d(density_map) / (M*N*Z)
        ajkl3 = torch_dct.dct(
                    torch_dct.dct(
                        torch_dct.dct(
                            density_map
                ...
                ).transpose(-1, -3).transpose(-1, -2) / (M*N*Z)
        if torch.allclose(ajkl, ajkl2, rtol=1e-3, atol=1e-5):
            print("ajkl = ajkl2")
        else:
            print("ajkl != ajkl2 #######")
        #"""

        ajkl_by_wj2_plus_wk2_plus_wl2_wj = ajkl.mul(wj_by_wj2_plus_wk2_plus_wl2_half)
        ajkl_by_wj2_plus_wk2_plus_wl2_wk = ajkl.mul(wk_by_wj2_plus_wk2_plus_wl2_half)
        ajkl_by_wj2_plus_wk2_plus_wl2_wl = ajkl.mul(wl_by_wj2_plus_wk2_plus_wl2_half)



        ctx.field_map_x = idxst.forward(
                            idct.forward(  
                                idct.forward(
                                    ajkl_by_wj2_plus_wk2_plus_wl2_wj
                                ).transpose(1,2).contiguous()
                            ).transpose(0,2).contiguous()
                        ).transpose(0,2).contiguous().transpose(1,2).contiguous()
        """验证一致
        ctx.field_map_x2 = idxst.forward(
                            idct.forward(
                                idct.forward(
                                    ajkl_by_wj2_plus_wk2_plus_wl2_wj
                                ).transpose(1,2).contiguous()
        ...
        ctx.field_map_x3 = ElectricPotentialFunction.idxst_post(
                            torch_dct.idct(
                                ElectricPotentialFunction.idxst_pre(t)
                                ),t
                            ).transpose(-1,-3).transpose(-1, -2) * M * N * Z * 8
        #"""
        """下面两个式子结果相同
        t = idxst.forward(ajkl_by_wj2_plus_wk2_plus_wl2_wj)
        t2 = ElectricPotentialFunction.idxst_post(
                idct.forward(ElectricPotentialFunction.idxst_pre(ajkl_by_wj2_plus_wk2_plus_wl2_wj)),
                ajkl_by_wj2_plus_wk2_plus_wl2_wj)
        #"""

        ctx.field_map_y = idct.forward(
                            idxst.forward(
                                idct.forward(
                                    ajkl_by_wj2_plus_wk2_plus_wl2_wk
                                ).transpose(1,2).contiguous()
                            ).transpose(0,2).contiguous()
                        ).transpose(0,2).contiguous().transpose(1,2).contiguous()
        
        ctx.field_map_z = idct.forward(
                            idct.forward(
                                idxst.forward(
                                    ajkl_by_wj2_plus_wk2_plus_wl2_wl
                                ).transpose(1,2).contiguous()
                            ).transpose(0,2).contiguous()
                        ).transpose(0,2).contiguous().transpose(1,2).contiguous()
        ...
        if fast_mode:  # dummy for invoking backward propagation
            ...
            # auv / (wu**2 + wv**2)
            ajkl_by_wj2_plus_wk2_plus_wl2 = ajkl.mul(inv_wj2_plus_wk2_plus_wl2)
            ...
            potential_map = idct.forward(
                                idct.forward(
                                    idct.forward(
                                        ajkl_by_wj2_plus_wk2_plus_wl2
                                    ).transpose(1,2).contiguous()
                                ).transpose(0,2).contiguous()
                            ).transpose(0,2).contiguous().transpose(1,2).contiguous()
            
            '''#与开源库验证过了，是一致的
            potential_map2 = torch_dct.idct_3d(ajkl_by_wj2_plus_wk2_plus_wl2) * M * N * Z * 8
            if torch.allclose(potential_map, potential_map2, rtol=1e-3, atol=1e-5):
                print("potential_map = potential_map2")
            else:
                print("potential_map != potential_map2 #######")
            #'''
            # compute energy
    ...
    def backward(ctx, grad_pos):
        ...
        tt = time.time()
        if grad_pos.is_cuda: # 未修改cuda版本
            output = -electric_potential_cuda.electric_force(
                grad_pos, ctx.num_bins_x, ctx.num_bins_y, ctx.num_bins_z,
                ctx.num_movable_impacted_bins_x,
                ctx.num_movable_impacted_bins_y,
                ctx.num_movable_impacted_bins_z,
                ctx.num_filler_impacted_bins_x, ctx.num_filler_impacted_bins_y, ctx.num_filler_impacted_bins_z,
                ctx.field_map_x.view([-1]), ctx.field_map_y.view([-1]), ctx.field_map_z.view([-1]), 
                ctx.pos, 
                ctx.node_size_x_clamped,
                ctx.node_size_y_clamped, 
                ctx.node_size_z_clamped,
                ctx.offset_x, ctx.offset_y, ctx.offset_z,
                ctx.ratio,
                ctx.bin_center_x, ctx.bin_center_y, ctx.bin_center_z,
                ctx.xl, ctx.yl, ctx.zl,
                ctx.xh, ctx.yh, ctx.zh,
                ctx.bin_size_x, ctx.bin_size_y, ctx.bin_size_z,
            ...
            output = -electric_potential_cpp.electric_force(
                ...
                ctx.num_movable_impacted_bins_z,
                ctx.num_filler_impacted_bins_x, ctx.num_filler_impacted_bins_y, ctx.num_filler_impacted_bins_z,
                ctx.field_map_x.view([-1]), ctx.field_map_y.view([-1]), ctx.field_map_z.view([-1]), ctx.pos, 
                ctx.node_size_x_clamped, ctx.node_size_y_clamped, ctx.node_size_z_clamped, 
                ctx.offset_x, ctx.offset_y, ctx.offset_z, ctx.ratio,
                ctx.bin_center_x, ctx.bin_center_y, ctx.bin_center_z, 
                ctx.xl, ctx.yl, ctx.zl, ctx.xh, ctx.yh, ctx.zh, 
                ctx.bin_size_x, ctx.bin_size_y, ctx.bin_size_z, 
                ctx.num_movable_nodes,
...
class ElectricPotential(ElectricOverflow):
    ...
    """
    def __init__(
        self,node_size_x, node_size_y, node_size_z,
        bin_center_x, bin_center_y, bin_center_z,
        target_density,xl, yl, zl, xh, yh, zh,
        bin_size_x, bin_size_y, bin_size_z,
        num_movable_nodes,num_terminals,num_filler_nodes,
        padding,deterministic_flag,  # control whether to use deterministic routine
        sorted_node_map, movable_macro_mask=None,fast_mode=False,region_id=None,
        fence_regions=None, # [n_subregion, 4] as dummy macros added to initial density. (xl,yl,xh,yh) rectangles
        node2fence_region_map=None,placedb=None):
        """
    ...
    def reset(self):
        ...
        self.exact_expkZ = None
        self.inv_wj2_plus_wk2_plus_wl2 = None
        self.wj_by_wj2_plus_wk2_plus_wl2_half = None
        self.wk_by_wj2_plus_wk2_plus_wl2_half = None
        self.wl_by_wj2_plus_wk2_plus_wl2_half = None
    ...
    def forward(self, pos, mode="density"):
        ...
        if self.initial_density_map is None:
            num_nodes = pos.size(0)// 3  # 3D modify 
            ...
            N = self.num_bins_y
            Z = self.num_bins_z
            # Compute exp(-j*pi*u/(2N)) = cos(pi*u/(2N)) - j * sin(pi*u/(2N))   u is variable
            # 有点难改
            ...
            self.exact_expkN = precompute_expk(N,
                                               dtype=pos.dtype,
                                               device=pos.device)
            self.exact_expkZ = precompute_expk(Z,
                                               dtype=pos.dtype,
                                               device=pos.device) # 3D modify 

            # init dct2, idct2, idct_idxst, idxst_idct with expkM and expkN
            self.dct = dct1d.DCT()
            self.idct = dct1d.IDCT()
            self.idxst = dct1d.IDXST()
            
            """
            self.dct2 = dct.DCT2(self.exact_expkM, self.exact_expkN, self.exact_expkZ) # 3D modify 
            if not self.fast_mode:
                self.idct2 = dct.IDCT2(self.exact_expkM, self.exact_expkN, self.exact_expkZ)
            self.idct_idxst = dct.IDCT_IDXST(self.exact_expkM,
                                             self.exact_expkN,
                                             self.exact_expkZ)
            self.idxst_idct = dct.IDXST_IDCT(self.exact_expkM, 
                                             self.exact_expkN,
                                             self.exact_expkZ)  # 3D modify 只改了声明部分
            """
            # wj, wk and wl
            wj = torch.arange(M, dtype=pos.dtype, device=pos.device).mul(2 * np.pi / M).view([M, 1, 1])
            # scale wv because the aspect ratio of a bin may not be 1
            ...
            wj2_plus_wk2_plus_wl2 = wj.pow(2) + wk.pow(2) + wl.pow(2)  # 矩阵会变成 M * N的矩阵，广播机制
            wj2_plus_wk2_plus_wl2[0, 0, 0] = 1.0  # avoid zero-division, it will be zeroed out
            self.inv_wj2_plus_wk2_plus_wl2 = 1.0 / wj2_plus_wk2_plus_wl2
            self.inv_wj2_plus_wk2_plus_wl2[0, 0, 0] = 0.0
            self.wj_by_wj2_plus_wk2_plus_wl2_half = wj.mul(self.inv_wj2_plus_wk2_plus_wl2).mul_(1. / 2)
            self.wk_by_wj2_plus_wk2_plus_wl2_half = wk.mul(self.inv_wj2_plus_wk2_plus_wl2).mul_(1. / 2)
            self.wl_by_wj2_plus_wk2_plus_wl2_half = wl.mul(self.inv_wj2_plus_wk2_plus_wl2).mul_(1. / 2)
        
        #使用density模式
        if(mode == "density"):
            return ElectricPotentialFunction.apply(  #3D modify
                ...
                self.bin_size_x, self.bin_size_y, self.bin_size_z,
                self.num_movable_nodes, self.num_filler_nodes, self.padding, self.padding_mask, 
                ...
                self.exact_expkM, self.exact_expkN, self.exact_expkZ, 
                self.inv_wj2_plus_wk2_plus_wl2,
                self.wj_by_wj2_plus_wk2_plus_wl2_half, self.wk_by_wj2_plus_wk2_plus_wl2_half,self.wl_by_wj2_plus_wk2_plus_wl2_half,
                self.dct, self.idct, self.idxst,
                self.fast_mode)
        #后面不用 未修改
        elif(mode == "overflow"):
            ...
            bin_area = self.bin_size_x * self.bin_size_y * self.bin_size_z
