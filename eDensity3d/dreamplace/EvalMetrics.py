...
class EvalMetrics (object):
    ...
    def evaluate(self, placedb, ops, var, data_collections=None):
        ...
        with torch.no_grad():
            ...
            if "overflow" in ops:
                overflow, max_density = ops["overflow"](var)  # var就是pos 
                if overflow.numel() == 1:
                    self.overflow = overflow.data / placedb.total_movable_node_area # 按百分比
                    self.overflow_orig = overflow.data
