...
#define REGISTER_KERNEL_LAUNCHER(T)                                            \
  template int computeElectricForceCudaLauncher<T>(                            \
      int num_bins_x, int num_bins_y, int num_bins_z, int num_impacted_bins_x,  \
      int num_impacted_bins_y, int num_impacted_bins_z, const T *field_map_x_tensor,                    \
      const T *field_map_y_tensor, const T *field_map_z_tensor, \
      const T *x_tensor, const T *y_tensor, const T *z_tensor,      \
      const T *node_size_x_clamped_tensor,                                     \
      const T *node_size_y_clamped_tensor, const T *node_size_z_clamped_tensor,\
      const T *offset_x_tensor,const T *offset_y_tensor,const T *offset_z_tensor, \
      const T *ratio_tensor,                         \
      const T *bin_center_x_tensor, const T *bin_center_y_tensor, const T *bin_center_z_tensor, \
      T xl, T yl, T zl, T xh, T yh, T zh,\
      T bin_size_x, T bin_size_y, T bin_size_z, int num_nodes, bool deterministic_flag, \
      T *grad_x_tensor, T *grad_y_tensor, T *grad_z_tensor, const int *sorted_node_map); 
