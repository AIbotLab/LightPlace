...
int computeMoveBoundaryMapCudaLauncher(
        T* x_tensor, T* y_tensor, T* z_tensor,
        const T* node_size_x_tensor,
        ...
        )
{
    ...
    cudaError_t status_z;
    cudaStream_t stream_z;
    status_z = cudaStreamCreate(&stream_z);
    if (status_z != cudaSuccess)
    {
        printf("cudaStreamCreate failed for stream_z\n");
        fflush(stdout);
        return 1;
    }
    ...
    computeMoveBoundary<<<block_count, thread_count, 0, stream_z>>>(
            z_tensor,
            node_size_z_tensor,
            zl, zh,
            num_nodes,
