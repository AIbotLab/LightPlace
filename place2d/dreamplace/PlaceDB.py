...
class PlaceDB (object):
    ...
    def __init__(self):
        ...
        ##### modify ####
        # self.node_z = None # 1D array, cell position z (0, 1)  
        # self.node_size_z = None # 1D array, cell z
        # self.zh = None
        # self.zl = None
        # self.routing_grid_zl = None
        # self.routing_grid_zh = None
        #################
    ...
    def read(self, params):
        ...
        self.rawdb = place_io.PlaceIOFunction.read(params)  # cpp读入文件
    ...
    def initialize_from_rawdb(self, params):
        ...
        #用bottom进行布局
        if params.preprocess_method == "half":
            #根号2
            for i in range(0, len(pydb.node_size_x)):
                pydb.node_size_x[i] = round(pydb.node_size_x[i] * pow(0.5, 0.5))
                pydb.node_size_y[i] = round(pydb.node_size_y[i] * pow(0.5, 0.5))

        elif params.preprocess_method == "halfWidth":
            #修改面积（宽度减半，行高不变）
            for i in range(0, len(pydb.node_size_x)):
                pydb.node_size_x[i] = pydb.node_size_x[i] / 2
        elif params.preprocess_method == "double":
            #长*1.5  宽*1.5  die面积变为2.25
            pydb.xh = int(pydb.xh * 1.5)
            pydb.yh = int(pydb.yh * 1.5)
            new_rows = list()
            turn = int(pydb.yh/pydb.row_height)
            for i in range(0, turn):
                new_rows.append((pydb.xl, i*pydb.row_height, pydb.xh, (i+1)*pydb.row_height))
            pydb.rows = new_rows
        ...
        if not use_read_pl_flag:
            ...
            #加入node_z 默认都在第一层，即全为0
            #self.node_z = np.zeros(pydb.num_nodes, dtype=self.dtype) 
        ...
        #默认厚度全为1  暂时无作用
        #self.node_size_z = np.full(pydb.num_nodes, 1.0, dtype=self.dtype)
        ...
        #加入z轴路由上下限
        # self.routing_grid_zl = float(0)
        # self.routing_grid_zh = float(1)
        ...
        # convert node2pin_map to array of array  根据每个node转换为单独的数组
        for i in range(len(self.node2pin_map)):
            self.node2pin_map[i] = np.array(self.node2pin_map[i], dtype=np.int32)
        self.node2pin_map = np.array(self.node2pin_map)

        # convert net2pin_map to array of array  根据每个net转换为单独的数组
        for i in range(len(self.net2pin_map)):
    ...
    def initialize(self, params):
        ...
        self.scale(params.shift_factor, params.scale_factor)  # 缩放模块
...
""" % (
        ...
        self.num_bins_x = max(params.num_bins_x, num_bins_x)  # bin的大小不一定与设置的有关
        self.num_bins_y = max(params.num_bins_y, num_bins_y)
        # set bin size 
        self.bin_size_x = (self.xh - self.xl) / self.num_bins_x  # 计算 每个bin的大小
        ...
        target_density = min(self.total_movable_node_area / self.total_space_area, 1.0)  # 类似于利用率  用于判断target density是否设置的过小
        ...
        if params.enable_fillers:
            ...
            if len(self.regions) > 0:
                ...
                node_size_order = np.argsort(self.node_size_x[: self.num_movable_nodes])  #按默认快排，排序
                # 去掉最高和最低的5% 然后求平均
                filler_size_x = np.mean(
                    self.node_size_x[
                ...
                self.total_filler_node_area = max(
                    placeable_area * params.target_density - self.total_movable_node_area, 0.0  #芯片上的可放置面积*目标密度 - 可移动的块的面积
                )
                self.num_filler_nodes = int(round(self.total_filler_node_area / (filler_size_x * filler_size_y)))  # 根据面积 计算出filler个数
                # 对x轴和y轴 增加 num_filler_nodes 个节点
