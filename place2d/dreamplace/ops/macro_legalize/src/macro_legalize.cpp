...
at::Tensor macro_legalization_forward(
  ...
  CHECK_FLAT_CPU(init_pos);  //判断是否在CPU上
  CHECK_EVEN(init_pos);    //必须是偶数个点
  CHECK_CONTIGUOUS(init_pos);  //判断点是否连续
...
bool macroLegalizationLauncher(LegalizationDB<T> db) {
  ...
  for (int i = 0; i < db.num_movable_nodes; ++i) {
    if (db.is_dummy_fixed(i)) {  //判断是否是宏块  大于两倍行高被认为是宏块  我们修改为大于1倍即被认定为宏块
  ...
  // if(!legal){
  //   printf("[ERROR  ] Place3D - legality check failed in macro legalization\n");
  //   //exit(2);
  // }
  // else{
  //   printf("[INFO   ] Place3D - legality check successful in macro legalization\n");
  // }
