...
int computeWeightedAverageWirelengthMergedLauncher(
    const T *x, const T *y, const T *z, const int *flat_netpin, const int *netpin_start,
    const unsigned char *net_mask, int num_nets, const T *inv_gamma,
    T *partial_wl, T *grad_intermediate_x, T *grad_intermediate_y, T *grad_intermediate_z,
    int num_threads, float zAlpha) {
  int chunk_size =
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, chunk_size)
  for (int i = 0; i < num_nets; ++i) {
    if (net_mask[i]) {
      ...
      T z_max = -std::numeric_limits<T>::max();
      ...
      for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
        ...
        // 不考虑z轴线长 x
        T zz = z[flat_netpin[j]];
        z_max = DREAMPLACE_STD_NAMESPACE::max(zz, z_max);  //3D modify
        z_min = DREAMPLACE_STD_NAMESPACE::min(zz, z_min);
      }
      ...
      for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
        T xx = x[flat_netpin[j]];
        T exp_x = exp((xx - x_max) * (*inv_gamma));  //对应论文中 a_i_+
        T exp_nx = exp((x_min - xx) * (*inv_gamma));  //对应论文中 a_i_-

        xexp_x_sum += xx * exp_x;   //对应论文中 c_e_+
        xexp_nx_sum += xx * exp_nx;  //对应论文中 c_e_-
        exp_x_sum += exp_x;   //对应论文中 b_e_+
        exp_nx_sum += exp_nx;   //对应论文中 b_e_-
        ...
        // 不考虑z轴线长 x
        T zz = z[flat_netpin[j]];
        T exp_z = exp((zz - z_max) * (*inv_gamma));
        T exp_nz = exp((z_min - zz) * (*inv_gamma));

        zexp_z_sum += zz * exp_z;
        zexp_nz_sum += zz * exp_nz;
        exp_z_sum += exp_z;
        exp_nz_sum += exp_nz;
      }
      // int zAlpha = 8; // 1 2 4 8 16 32
      // int zAlpha = 1;
      partial_wl[i] = xexp_x_sum / exp_x_sum - xexp_nx_sum / exp_nx_sum +
                      yexp_y_sum / exp_y_sum - yexp_ny_sum / exp_ny_sum +
                      (zexp_z_sum / exp_z_sum - zexp_nz_sum / exp_nz_sum) * zAlpha ;  // 1 / zAlpha; //modify
      // printf("i:%d, partial_wl:%f", i, partial_wl[i]);

      T b_x = (*inv_gamma) / (exp_x_sum);  //对应论文中 1/(gamma * b_e_+)
      ...
      // 不考虑z轴线长 x
      T b_z = (*inv_gamma) / (exp_z_sum);
      T a_z = (1.0 - b_z * zexp_z_sum) / exp_z_sum;
      T b_nz = -(*inv_gamma) / (exp_nz_sum);
      T a_nz = (1.0 - b_nz * zexp_nz_sum) / exp_nz_sum;
      ...
      for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
        ...
        grad_intermediate_x[flat_netpin[j]] =
            (a_x + b_x * xx) * exp_x - (a_nx + b_nx * xx) * exp_nx; //对应论文公式 6
        ...
        grad_intermediate_z[flat_netpin[j]] = 
            ((a_z + b_z * zz) * exp_z - (a_nz + b_nz * zz) * exp_nz) * zAlpha ; // 1 / zAlpha; // modify
      }
    }
  }
...
template <typename T>
int computeWeightedAverageWirelengthMergedLauncher2d(
...
std::vector<at::Tensor> weighted_average_wirelength_forward(
    ...
    at::Tensor inv_gamma, float zAlpha) {
  CHECK_FLAT_CPU(pos);
  CHECK_EVEN(pos);
  ...
  int num_pins = pos.numel() / 3; //3D modify

  // x, y interleave
  // at::Tensor partial_wl = at::zeros({num_nets, 3}, pos.options());  //是否需要变成3  目前觉得不需要
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computeWeightedAverageWirelengthMergedLauncher", [&] {
        computeWeightedAverageWirelengthMergedLauncher<scalar_t>(
            ...
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_pins*2,
            ...
            DREAMPLACE_TENSOR_DATA_PTR(grad_intermediate, scalar_t) + num_pins*2,
            at::get_num_threads(),
            zAlpha
            );
...
std::vector<at::Tensor> weighted_average_wirelength_forward_2d(
    at::Tensor pos, at::Tensor flat_netpin, at::Tensor netpin_start,
    at::Tensor pin2net_map, at::Tensor net_weights, at::Tensor net_mask,
    at::Tensor inv_gamma) {
  CHECK_FLAT_CPU(pos);
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computeWeightedAverageWirelengthMergedLauncher2d", [&] {
        computeWeightedAverageWirelengthMergedLauncher2d<scalar_t>(
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_pins,
            DREAMPLACE_TENSOR_DATA_PTR(flat_netpin, int),
            DREAMPLACE_TENSOR_DATA_PTR(netpin_start, int),
...
at::Tensor weighted_average_wirelength_backward(
  ...
  int pin_nums = pos.numel() / 3; //3D modify
      ...
      pos, "computeWeightedAverageWirelengthMergedLauncher", [&] {
        if (net_weights.numel()) {
          integrateNetWeightsLauncher<scalar_t>(
              ...
              DREAMPLACE_TENSOR_DATA_PTR(grad_out, scalar_t) + pin_nums,
              DREAMPLACE_TENSOR_DATA_PTR(grad_out, scalar_t) + pin_nums*2,
...
at::Tensor weighted_average_wirelength_backward_2d(
    at::Tensor grad_pos, at::Tensor pos, at::Tensor grad_intermediate,
    at::Tensor flat_netpin, at::Tensor netpin_start, at::Tensor pin2net_map,
    at::Tensor net_weights, at::Tensor net_mask, at::Tensor inv_gamma) {
  CHECK_FLAT_CPU(pos);
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computeWeightedAverageWirelengthMergedLauncher2d", [&] {
        if (net_weights.numel()) {
          integrateNetWeightsLauncher2d<scalar_t>(
              DREAMPLACE_TENSOR_DATA_PTR(flat_netpin, int),
              DREAMPLACE_TENSOR_DATA_PTR(netpin_start, int),
              DREAMPLACE_TENSOR_DATA_PTR(net_mask, unsigned char),
              DREAMPLACE_TENSOR_DATA_PTR(net_weights, scalar_t),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("forward2d", &DREAMPLACE_NAMESPACE::weighted_average_wirelength_forward_2d,
        "WeightedAverageWirelength forward 2d");
  m.def("backward2d", &DREAMPLACE_NAMESPACE::weighted_average_wirelength_backward_2d,
        "WeightedAverageWirelength backward 2d");
}
