...
class BasicPlace(nn.Module):
    ...
    def __init__(self, params, placedb):
        ...
        if params.global_place_flag:
            ...
            if params.random_center_init_flag == 2: # 2 的时候是随机分散在die上
            ...
            if params.random_center_init_flag == 1:  # move to center of layout
                self.init_pos[placedb.num_nodes:placedb.num_nodes +
                            placedb.num_movable_nodes] = np.random.normal(
                                ...
                                size=placedb.num_movable_nodes)
            if params.random_center_init_flag == 2:
                self.init_pos[placedb.num_nodes:placedb.num_nodes +
        ...
        if len(placedb.regions) > 0:
            ...
            self.op_collections.legalize_op = self.build_legalization(   # 合法化在此
            params, placedb, self.data_collections, self.device)
        # detailed placement
    ...
    def build_legalization(self, params, placedb, data_collections, device):
        ...
        def build_legalization_op(pos, legalList, onlyMacroLgFlag = 0):
            logging.info("Start legalization [in function build_legalization_op]")
            pos1 = ml(pos, pos)
            
            if onlyMacroLgFlag:
                macroLegal = self.op_collections.legality_check_op(pos1, onlyMacroLgFlag) # 只做宏块重叠检查
                legalList[0] = macroLegal
                return pos1 #只做宏块合法化
            pos2 = gl(pos1, pos1)
            legal = self.op_collections.legality_check_op(pos2)
            legalList[1] = legal
            if not legal:
                logging.info("legality check failed in greedy legalization")
                # # 直接退出
                # exit(3)
                return pos2
            return al(pos1, pos2)
    ...
    def plot(self, params, placedb, iteration, pos, phase = None):
        """
        @brief plot layout
        @param params parameters
        @param placedb placement database
        ...
        if phase != None:
            figname = "%s/plot/iter_%s.png" % (path, phase)
