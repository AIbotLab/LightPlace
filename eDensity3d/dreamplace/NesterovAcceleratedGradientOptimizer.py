        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group['params']):
                ...
                if not group['a_k']:
                    ...
                    group['v_k_1'][i].data.copy_(group['v_k'][i]-group['lr']*g_k)  # 因为lr为0
                    obj, grad = obj_and_grad_fn(group['v_k_1'][i])
                    group['g_k_1'].append(grad.data)  # g_k_1为什么会和g_k完全一致
                ...
                if not group['alpha_k']:
                    group['alpha_k'].append((v_k-v_k_1).norm(p=2) / (g_k-g_k_1).norm(p=2))  # 可能存在除0的情况
