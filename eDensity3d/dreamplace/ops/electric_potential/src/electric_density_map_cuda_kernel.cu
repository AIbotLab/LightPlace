...
__global__ void __launch_bounds__(1024, 8) computeTriangleDensityMap(
    ...
    const T *node_size_z_clamped_tensor,
    const T *offset_x_tensor,
    const T *offset_y_tensor, 
    const T *offset_z_tensor, 
    ...
    const T half_bin_size_x, const T half_bin_size_y, const T half_bin_size_z, 
    const T bin_size_x, const T bin_size_y, const T bin_size_z,
    const T inv_bin_size_x, const T inv_bin_size_y, const T inv_bin_size_z, 
    AtomicOp atomic_add_op,
    typename AtomicOp::type *density_map_tensor,
    const int *sorted_node_map  ///< can be NULL if not sorted
) {
  int index = blockIdx.x * blockDim.z + threadIdx.z;  //因为grid只有1维，所以blockIdx.x恒为0，式子等于threadIdx.z
  ...
  if (index < num_nodes) {
    ...
    T node_size_z = node_size_z_clamped_tensor[i]; //3D modify
    ...
    int bin_index_zh = int(((node_z + node_size_z - zl) * inv_bin_size_z)) + 1;  // exclusive
    ...
    for (int k = bin_index_xl + threadIdx.y; k < bin_index_xh; k += blockDim.y) { //不清楚thredIdx.y与x维度相加的含义，但是因为是2*2的，没有区别
      T px = triangle_density_function(node_x, node_size_x, xl, k, bin_size_x);  //计算模块与bin的重叠
      ...
      for (int h = bin_index_yl + threadIdx.x; h < bin_index_yh; h += blockDim.x) {
        ...
        //3D modify
        for (int f = bin_index_zl + threadIdx.x; f < bin_index_zh; f += blockDim.x ) { //接着用thredIdx.x即可
          T pz = triangle_density_function(node_z, node_size_z, zl, f, bin_size_z);
          T volum = area * pz;
          atomic_add_op(&density_map_tensor[k * num_bins_y * num_bins_z + h * num_bins_z + f], volum);
        }
...
__global__ void computeExactDensityMapCellByCell(
    ...
    const T *bin_center_x_tensor, const T *bin_center_y_tensor, const T *bin_center_z_tensor,
    const int num_nodes, 
    const int num_bins_x, const int num_bins_y, const int num_bins_z, 
    const T xl, const T yl, const T zl, const T xh, const T yh, const T zh,
    const T bin_size_x, const T bin_size_y, const T bin_size_z,
    const int num_impacted_bins_x, const int num_impacted_bins_y, const int num_impacted_bins_z,
    bool fixed_node_flag, AtomicOp atomic_add_op,
    typename AtomicOp::type *density_map_tensor
    ) {
  ...
  auto box2bin = [&](T bxl, T byl, T bzl, T bxh, T byh, T bzh) {
    ...
    for (int k = bin_index_xl; k < bin_index_xh; ++k) {
      ...
      for (int h = bin_index_yl; h < bin_index_yh; ++h) {
        T py = exact_density_function(byl, byh - byl, bin_center_y_tensor[h],
                                      bin_size_y, yl, yh, fixed_node_flag);
        for (int t = bin_index_zl; t < bin_index_zh; ++t) {
          T pz = exact_density_function(bzl, bzh - bzl, bin_center_z_tensor[t],
                                        bin_size_z, zl, zh, fixed_node_flag);
          //3D modify
          // still area  volume
          atomic_add_op(&density_map_tensor[ k * num_bins_y * num_bins_z + h * num_bins_y + t],
                         px * py * pz);                          
        }  
      }
    }
...
int computeTriangleDensityMapCallKernel(
    ...
    const T *bin_center_y_tensor, const T *bin_center_z_tensor, int num_nodes,
    const int num_bins_x, const int num_bins_y, const int num_bins_z,
    int num_impacted_bins_x, int num_impacted_bins_y, int num_impacted_bins_z, 
    const T xl, const T yl, const T zl, const T xh, const T yh, const T zh, 
    const T bin_size_x, const T bin_size_y, const T bin_size_z, AtomicOp atomic_add_op,
    typename AtomicOp::type *density_map_tensor, const int *sorted_node_map) {
  int thread_count = 64;
  dim3 blockSize(2, 2, thread_count);  //没改过这行
  ...
  computeTriangleDensityMap<<<block_count, blockSize>>>(
      ...
      node_size_x_clamped_tensor,
      node_size_y_clamped_tensor, 
      node_size_z_clamped_tensor,
      offset_x_tensor, offset_y_tensor, offset_z_tensor,
      ratio_tensor, bin_center_x_tensor, bin_center_y_tensor, bin_center_z_tensor,
      num_nodes, num_bins_x, num_bins_y, num_bins_z,
      xl, yl, zl, xh, yh, zh, bin_size_x / 2, bin_size_y / 2, bin_size_z / 2,
      bin_size_x, bin_size_y, bin_size_z, 1 / bin_size_x, 1 / bin_size_y, 1 / bin_size_z, atomic_add_op,
...
template <typename T>
int computeTriangleDensityMapCudaLauncher( //3D modify
    const T *x_tensor, const T *y_tensor, const T *z_tensor, 
    const T *node_size_x_clamped_tensor,
    const T *node_size_y_clamped_tensor, 
    const T *node_size_z_clamped_tensor, 
    ...
    int num_impacted_bins_x, int num_impacted_bins_y, int num_impacted_bins_z, 
    const T xl, const T yl, const T zl, const T xh, const T yh, const T zh,
    const T bin_size_x, const T bin_size_y, const T bin_size_z, bool deterministic_flag,
  ...
  {
    // total die area  3D modify
    //double diearea = (xh - xl) * (yh - yl);
    double dievolume = (xh - xl) * (yh - yl) * (zh - zl);
    int integer_bits = max((int)ceil(log2(dievolume)) + 1, 32);
    ...
    computeTriangleDensityMapCallKernel<T, decltype(atomic_add_op)>(  //3D modify
        x_tensor, y_tensor, z_tensor, 
        node_size_x_clamped_tensor,
        node_size_y_clamped_tensor, 
        node_size_z_clamped_tensor, 
        offset_x_tensor, offset_y_tensor, offset_z_tensor, ratio_tensor,
        bin_center_x_tensor, bin_center_y_tensor, bin_center_z_tensor, num_nodes,
        num_bins_x, num_bins_y, num_bins_z, num_impacted_bins_x, num_impacted_bins_y, num_impacted_bins_z, 
        xl, yl, zl, xh, yh, zh, bin_size_x, bin_size_y, bin_size_z, atomic_add_op,
...
int computeExactDensityMapCallKernel(
  ...
  computeExactDensityMapCellByCell<<<block_count, thread_count>>>(
      x_tensor, y_tensor, z_tensor,
      node_size_x_tensor, node_size_y_tensor, node_size_z_tensor,
      bin_center_x_tensor, bin_center_y_tensor, bin_center_z_tensor,
      num_nodes, 
      num_bins_x, num_bins_y, num_bins_z,
      xl, yl, zl, xh, yh, zh,
      bin_size_x, bin_size_y, bin_size_z,
      num_impacted_bins_x, num_impacted_bins_y, num_impacted_bins_z,
      fixed_node_flag, atomic_add_op, density_map_tensor
      );
...
int computeExactDensityMapCudaLauncher(
                           ...
                           // as fixed point number
  {
    ...
    int thread_count = 512;
    // 将density_map_tensor 缩放至 scale_factor 倍数, 然后存储在 scaled_density_map_tensor 当中
    copyScaleArray<<<(num_bins + thread_count - 1) / thread_count,
                     thread_count>>>(
        scaled_density_map_tensor, density_map_tensor, scale_factor, num_bins);
