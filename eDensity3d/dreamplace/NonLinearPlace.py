...
class Terminal(object):
    def __init__(self, placedb):
        ...
        self.num_bins_y = placedb.num_bins_x 
...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    def __call__(self, params, placedb):
        ...
        """
        iteration = 0
        best_iteration = 0 # 记录最好的GP迭代结果位置
        all_metrics = []
        gamma_record = []
        density_weight_record = []
        energy_record = []
        best_cur_metric = None # 记录最好的GP迭代指标
        ...
        if params.global_place_flag:
            ...
            for global_place_params in params.global_place_stages:
                ...
                if optimizer_name.lower() == "adam":
                    optimizer = torch.optim.Adam(self.parameters(), lr=0.5) # 原本是0.05
                    ...
                    optimizer = NesterovAcceleratedGradientOptimizer.NesterovAcceleratedGradientOptimizer(
                        self.parameters(),
                        lr=0.1,
                        obj_and_grad_fn=model.obj_and_grad_fn, # mark
                ...
                def initialize_learning_rate(pos):
                    learning_rate = model.estimate_initial_learning_rate(  #改在这
                        pos, global_place_params["learning_rate"]
                    )
                    # print("initialize_learning_rate:",learning_rate)
                    # update learning rate
                    for param_group in optimizer.param_groups:
                        param_group["lr"] = learning_rate.data   # 学习率在此修改 存在0的情况
                ...
                if iteration == 0:
                    ...
                    initialize_learning_rate(model.data_collections.pos[0])  # 防止params.gp_noise_ratio 为0的时候 没有初始化学习率
                ...
                def one_descent_step(Lgamma_step, Llambda_density_weight_step, Lsub_step, iteration, metrics, stop_mask=None):
                    ...
                    # print("model.data_collections.pos[0]:", model.data_collections.pos[0])
                    ...
                    if "set_to_none" in inspect.signature(optimizer.zero_grad).parameters: 
                        ...
                        optimizer.zero_grad()  # 将优化器的梯度情况清空

                    # t1 = time.time()
                    cur_metric.evaluate(placedb, eval_ops, pos, model.data_collections) # 评估实际值
                    model.overflow = cur_metric.overflow.data.clone()
                    # logging.debug("evaluation %.3f ms" % ((time.time()-t1)*1000))
                    # t2 = time.time()
                    # as nesterov requires line search, we cannot follow the convention of other solvers
                    if optimizer_name.lower() in ["sgd", "adam", "sgd_momentum", "sgd_nesterov"]:
                        obj, grad = model.obj_and_grad_fn(pos)  # 前向和反向
                        cur_metric.objective = obj.data.clone()
                    elif optimizer_name.lower() != "nesterov":
                        assert 0, "unsupported optimizer %s" % (optimizer_name)
                    # plot placement
                    interval = 100
                    if hasattr(params, "draw_iter_interval"):
                        interval = params.draw_iter_interval
                    # if params.plot_flag == 1 and (iteration % 100 == 0 or iteration == 999):
                    # if params.plot_flag == 1 and (iteration % 10 == 0): # 每隔10次画一次图
                    if params.plot_flag == 1 and ( (iteration % 100 == 0) or (iteration % interval == 0 and iteration < 500)): 
                        cur_pos = self.pos[0].data.clone().cpu().numpy()
                        self.plot(params, placedb, iteration, cur_pos, params.draw_mode)
                    ...
                    if model.update_mask is not None:
                        ...
                        # print("iteration in step:",iteration)
                        # print("learning rate:",optimizer.param_groups[0]['lr'])
                        optimizer.step()
                    # logging.info("optimizer step %.3f ms" % ((time.time() - t3) * 1000))
                    # print("model.data_collections.pos[0] after:", model.data_collections.pos[0])
                    ...
                    gamma_record.append(cur_metric.gamma.item())
                    density_weight_record.append(cur_metric.density_weight.item())
                    ...
                    return cur_metric.hpwl, cur_metric.overflow
                ...
                def entropy_injection(pos, placedb, shrink_factor=1, noise_intensity=1, mode="random", iteration=1):
                    #3D modify
                    if mode == "random":
                        ...
                        zc = pos.data[placedb.num_nodes *2 : placedb.num_nodes*2 + placedb.num_movable_nodes].mean()
                        ...
                        fixed_pos_z = pos.data[
                            num_nodes*2 + num_movable_nodes : num_nodes*2 + num_movable_nodes + num_fixed_nodes
                        ].clone()

                        if shrink_factor != 1:
                            pos.data[:num_nodes] = (pos.data[:num_nodes] - xc) * shrink_factor + xc
                            pos.data[num_nodes:num_nodes*2] = (pos.data[num_nodes:num_nodes*2] - yc) * shrink_factor + yc
                            pos.data[num_nodes*2:] = (pos.data[num_nodes*2:] - zc) * shrink_factor + zc
                        if noise_intensity > 0.01:
                            # pos.data.add_(noise_intensity * torch.rand(num_nodes*2, device=pos.device).sub_(0.5))
                            pos.data.add_(noise_intensity * torch.randn(num_nodes * 3, device=pos.device)) #3D modify
                        ...
                        pos.data[num_nodes*2 + num_movable_nodes : num_nodes*2 + num_movable_nodes + num_fixed_nodes] = fixed_pos_z
                        # print(pos[: placedb.num_movable_nodes].mean())
                    else:
                ...
                curHPWL, curoverflow = 0 , 0 # 用于tqdm输出
                # pbar = tqdm(range(model.Lgamma_iteration)
                with trange(model.Lgamma_iteration) as pbar:
                    for Lgamma_step in pbar:
                        ...
                        for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):
                            ...
                            for Lsub_step in range(model.Lsub_iteration):
                                ...
                                curHPWL, curoverflow = one_descent_step(
                                    Lgamma_step, Llambda_density_weight_step, Lsub_step, iteration, Lsub_metrics
                                )
                                pbar.set_description("eDensity3d iter%d" % Lgamma_step)
                                pbar.set_postfix(str="HPWL:%.6E,overflow:%.6E" % (curHPWL,curoverflow))
                                # print("curHPWL:",curHPWL," curoverflow:", curoverflow)
                            ...
                            if Llambda_flat_iteration > 1:
                                ...
                                # print("update_density_weight_op ########")
                            ...
                            if (params.routability_opt_flag and num_area_adjust < params.max_num_area_adjust
                        ...
                        if optimizer_name.lower() in ["sgd", "adam", "sgd_momentum", "sgd_nesterov", "cg"]:
                            if "learning_rate_decay" in global_place_params:
                                ...
                                for param_group in optimizer.param_groups:
                                    ...
                                    print("learning_rate_decay:",param_group["lr"])
                    #
        ...
        # 输出最好的GP迭代
        # logging.info("The best result in GP is :%d" % (best_iteration))
        # logging.info(best_cur_metric)
        # self.pos[0] = 

        #分层 四舍五入
        tt = time.time()
        nums_node = placedb.num_nodes
        gp_pos = self.pos[0].data.clone()
        ori_pos = self.pos[0].data.clone()
        
        # 将每一层四舍五入
        cell_depth = placedb.cell_depth
        half_cell_depth = cell_depth / 2
        level_centers = [(i + 0.5) * cell_depth for i in range(placedb.layer)]
        def assign_to_nearest_level(point, levels):
            nearest_level = min(range(levels), key=lambda x: abs(point - level_centers[x]))
            return nearest_level
        gp_pos = gp_pos.numpy()
        # 得到分层数
        gp_pos[nums_node*2:] = [assign_to_nearest_level(point + half_cell_depth, placedb.layer) for point in gp_pos[nums_node*2:]]
        placedb.node2layer = gp_pos[nums_node*2:]  # node在每层的下标
        placedb.node2layer = placedb.node2layer.astype(dtype=np.int16)  # [0,2,0,1,2,2,1,1,0]
        gp_pos[nums_node*2:] = gp_pos[nums_node*2:] * cell_depth # 还原回真实z轴高度
        gp_pos = torch.from_numpy(gp_pos)
        # self.pos[0].data.copy_(gp_pos)
        '''
        placedb.layer2nodes = {}  # 每层含有的node的下标 {0:[0,2,8],1:[3,6,7],2:[1,4,5]}
        placedb.layer_num_fillers = {} #记录划分后每层的filler数量
        for node,layer in enumerate(placedb.node2layer):
            if layer not in placedb.layer2nodes:
                placedb.layer2nodes[layer] = []
                placedb.layer_num_fillers[layer] = 0
            placedb.layer2nodes[layer].append(node)  
            if node >= placedb.num_physical_nodes:
                placedb.layer_num_fillers[layer] += 1
        '''
            

        
        logging.info("tier partition takes %.3f seconds" % (time.time() - tt))
        cur_metric = EvalMetrics.EvalMetrics(iteration)
        cur_metric.evaluate(placedb, {"hpwl": self.op_collections.hpwl_op}, gp_pos)
        print("[INFO   ] Place3D - GP HPWL")
        print(cur_metric)
        
        # plot placement
        if params.plot_flag == 1 or params.plot_flag == 2:
            self.plot(params, placedb, iteration, gp_pos.data.clone().cpu().numpy(), params.draw_mode) # self.pos[0] 改为 gp_pos
        # save results
        cur_pos = self.pos[0].data.clone().cpu().numpy()
        # apply solution
        placedb.apply(
            ...
            cur_pos[placedb.num_nodes * 2 : placedb.num_nodes * 2 + placedb.num_movable_nodes] # 3D modify
        )
        
        return all_metrics, density_weight_record, gamma_record, placedb.node2layer, ori_pos[nums_node*2:] 
