...
int computeElectricForceCudaLauncher(
    ...
    T xl, T yl, T zl, T xh, T yh, T zh, T bin_size_x, T bin_size_y, T bin_size_z, 
    int num_nodes, bool deterministic_flag, 
    T *grad_x_tensor, T *grad_y_tensor, T *grad_z_tensor, const int *sorted_node_map);
...
at::Tensor electric_force(
  ...
  CHECK_FLAT_CUDA(field_map_z);
  CHECK_CONTIGUOUS(field_map_z);
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computeElectricForceCudaLauncher", [&] {
        computeElectricForceCudaLauncher<scalar_t>(
            ...
            xl, yl, zl, xh, yh, zh,
            bin_size_x, bin_size_y, bin_size_z, num_movable_nodes, (bool)deterministic_flag,
    ...
    DREAMPLACE_DISPATCH_FLOATING_TYPES(
        pos, "computeElectricForceCudaLauncher", [&] {
          computeElectricForceCudaLauncher<scalar_t>(
              ...
              DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_nodes*2 + num_physical_nodes,
              ...
              DREAMPLACE_TENSOR_DATA_PTR(bin_center_z, scalar_t),
              xl, yl, xl, xh, yh, zh, 
              bin_size_x, bin_size_y, bin_size_z, num_filler_nodes, (bool)deterministic_flag,
              ...
              DREAMPLACE_TENSOR_DATA_PTR(grad_out, scalar_t) + num_nodes*2 + num_physical_nodes,
              NULL);
        });
  }
