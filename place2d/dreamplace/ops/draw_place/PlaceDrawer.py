...
class PlaceDrawer(object):
    ...
    def forward(pos,
        ...
        try:
            ...
            for i in range(num_movable_nodes, num_movable_nodes + num_terminals):
            ...
            # draw fixed NI  设置成黄色
            ''' # 暂时不画出来
            ctx.set_source_rgba(238 / 255.0,
                                255 / 255.0,
                                34 / 255.0, alpha=0.5)
            for i in range(num_movable_nodes + num_terminals, num_physical_nodes):
                ctx.rectangle(node_xl[i], node_yl[i], node_xh[i] - node_xl[i],
                              node_yh[i] -
                              node_yl[i])  # Rectangle(xl, yl, w, h)
                ctx.fill()
