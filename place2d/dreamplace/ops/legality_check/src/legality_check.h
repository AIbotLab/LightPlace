...
bool legalityCheckKernelCPU(const T* x, const T* y, const T* node_size_x,
                            ...
                            const int num_movable_nodes,
                            const int num_regions, bool onlyMacro) {
  bool legal_flag = true;
  int num_rows = ceil((yh - yl) / row_height);  //算出有多少行是可用行
  dreamplaceAssert(num_rows > 0);
  fflush(stdout);
  std::vector<std::vector<int> > row_nodes(num_rows);
  ...
  // printf("boundaryCheck legal_flag:%d\n", legal_flag);
  ...
  // printf("siteAlignmentCheck legal_flag:%d\n", legal_flag);

  if (!overlapCheck(node_size_x, node_size_y, x, y, row_height, scale_factor,
                    xl, yl, xh, yh, num_nodes, num_movable_nodes)) {
    legal_flag = false;
  }
  // printf("overlapCheck legal_flag:%d\n", legal_flag);
  // check fence regions
  ...
  // printf("fenceRegionCheck legal_flag:%d\n", legal_flag);
  return legal_flag;
}
