...
template <typename T>
int computeHPWLLauncher(const T* x, const T* y, const T* z, const int* flat_netpin,
                        const int* netpin_start, const unsigned char* net_mask,
                        int num_nets, int num_threads, T* hpwl, float zAlpha);
...
at::Tensor hpwl_forward(at::Tensor pos, at::Tensor flat_netpin,
                        ...
                        at::Tensor net_mask, float zAlpha) {
  CHECK_FLAT_CPU(pos);
  CHECK_EVEN(pos);
...
#pragma omp parallel for num_threads(num_threads)
  for (int i = 0; i < num_nets; ++i) {
    ...
    T max_z = -std::numeric_limits<T>::max();
    ...
    if (net_mask[i]) {
      for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
        ...
        // 不考虑z轴线长  x
        min_z = std::min(min_z, z[flat_netpin[j]]);
        max_z = std::max(max_z, z[flat_netpin[j]]);
      }
      hpwl[i] = max_x - min_x + max_y - min_y + max_z * zAlpha - min_z * zAlpha ;
    }
  }
