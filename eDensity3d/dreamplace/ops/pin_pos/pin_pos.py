...
class PinPosFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, pin_offset_x, pin_offset_y, pin_offset_z, pin2node_map,
        ...
        output = func(ctx.pos, pin_offset_x, pin_offset_y, pin_offset_z,
                      pin2node_map, flat_node2pin_map, flat_node2pin_start_map)
        ctx.pin_offset_x = pin_offset_x
        ctx.pin_offset_y = pin_offset_y
        ctx.pin_offset_z = pin_offset_z  # 3D modify
    ...
    def backward(ctx, grad_pin_pos):
        ...
        output = func(grad_pin_pos.contiguous(), ctx.pos, ctx.pin_offset_x,
                      ctx.pin_offset_y,ctx.pin_offset_z, ctx.pin2node_map,
...
class PinPos(nn.Module):
    ...
    def __init__(self,
        ...
        self.pin_offset_z = pin_offset_z
    ...
    def forward(self, pos):
        ...
        """
        assert pos.numel() % 3 == 0
        num_nodes = pos.numel() // 3  # 3D modify
        if pos.is_cuda:
