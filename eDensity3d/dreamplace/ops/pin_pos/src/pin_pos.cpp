...
template <typename T>
int computePinPosLauncher(const T* x, const T* y, const T* z, const T* pin_offset_x,
                          const T* pin_offset_y, const T* pin_offset_z, const long* pin2node_map,
                          const int* flat_node2pin_map,
                          const int* flat_node2pin_start_map, int num_pins,
                          const int num_threads, T* pin_x, T* pin_y, T* pin_z);

template <typename T>
int computePinPosGradLauncher(const T* grad_out_x, const T* grad_out_y, const T* grad_out_z,
                              const T* x, const T* y, const T* z, const T* pin_offset_x,
                              const T* pin_offset_y, const T* pin_offset_z, const long* pin2node_map,
                              const int* flat_node2pin_map,
...
at::Tensor pin_pos_forward(at::Tensor pos, at::Tensor pin_offset_x,
  ...
  auto out = at::zeros(pin_offset_x.numel() * 3, pos.options());
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(pos, "computePinPosLauncher", [&] {
    computePinPosLauncher<scalar_t>(
        ...
        DREAMPLACE_TENSOR_DATA_PTR(out, scalar_t) + num_pins*2);
  });
...
at::Tensor pin_pos_backward(at::Tensor grad_out, at::Tensor pos,
                            at::Tensor pin_offset_x, at::Tensor pin_offset_y, at::Tensor pin_offset_z,
                            at::Tensor pin2node_map,
                            at::Tensor flat_node2pin_map,
                            at::Tensor flat_node2pin_start_map,
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computePinPosGradLauncher", [&] {
        computePinPosGradLauncher<scalar_t>(
            ...
            DREAMPLACE_TENSOR_DATA_PTR(grad_out, scalar_t) + num_pins*2,
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t),
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_nodes,
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t) + num_nodes*2,
            DREAMPLACE_TENSOR_DATA_PTR(pin_offset_x, scalar_t),
...
#pragma omp parallel for num_threads(num_threads)
  for (int i = 0; i < num_pins; ++i) {
    ...
    pin_z[i] = pin_offset_z[i] + z[node_id];
  }
