...
class PlaceDataCollection(object):
    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            self.node_size_z = torch.from_numpy(placedb.node_size_z).to(device) # 3D modify
            ...
            if params.routability_opt_flag:
                ...
                self.original_node_size_z = self.node_size_z.clone() # 3D modify
            ...
            self.pin_offset_y = torch.tensor(placedb.pin_offset_y,
                                             dtype=self.pos[0].dtype,
                                             device=device)
            self.pin_offset_z = torch.tensor(placedb.pin_offset_z,
                                             dtype=self.pos[0].dtype,
                                             device=device) # 3D modify
            ...
            if params.routability_opt_flag:
                ...
                self.original_pin_offset_z = self.pin_offset_z.clone() # 3D modify
            ...
            self.node_areas = self.node_size_x * self.node_size_y * self.node_size_z # 3D modify
            ...
            self.net_mask_all = torch.from_numpy(
                np.ones(placedb.num_nets,dtype=np.uint8)).to(device)  # all nets included
    ...
    def bin_center_z_padded(self, placedb, padding, num_bins_z):
        """
        @brief compute array of bin center horizontal coordinates with padding
        @param placedb placement database
        @param padding number of bins padding to boundary of placement region
        """
        bin_size_z = (placedb.zh - placedb.zl) / num_bins_z
        zl = placedb.zl - padding * bin_size_z
        zh = placedb.zh + padding * bin_size_z
        bin_center_z = torch.from_numpy(
            placedb.bin_centers(zl, zh, bin_size_z)).to(self.device)
        return bin_center_z
...
class PlaceOpCollection(object):
    ...
    def __init__(self):
        ...
        self.hpwl_op = None    # 这个算的是真实的HPWL线长
        ...
        self.wirelength_op = None   # 这个算的是近似的op，是为WA或者LSE近似模型
...
class BasicPlace(nn.Module):
    ...
    def __init__(self, params, placedb, timer):
        ...
        tt = time.time()
        self.init_pos = np.zeros(placedb.num_nodes * 3, dtype=placedb.dtype)  # 3D modify
        #self.init_pos_z = np.zeros(placedb.num_nodes, dtype=placedb.dtype)  
        # x position
        self.init_pos[0:placedb.num_physical_nodes] = placedb.node_x
        if params.global_place_flag:
            if params.random_center_init_flag == 1:  # move to center of layout
                logging.info(
            ...
            ## 完全随机
            elif params.random_center_init_flag == 2: # 2 的时候是随机分散在die上
            ...
            if params.random_center_init_flag == 1:  # move to center of layout
                self.init_pos[placedb.num_nodes:placedb.num_nodes +
                            placedb.num_movable_nodes] = np.random.normal(
                                ...
                                size=placedb.num_movable_nodes)
            elif params.random_center_init_flag == 2:
                self.init_pos[placedb.num_nodes:placedb.num_nodes +
        ...
        # z position  3D modify
        #self.init_pos_z[0:placedb.num_nodes] = placedb.node_z
        if params.global_place_flag:  # move to center of layout
            if params.random_center_init_flag == 1:  # move to center of layout
                self.init_pos[placedb.num_nodes * 2: placedb.num_nodes * 2 + 
                              placedb.num_movable_nodes] = np.random.normal(
                                loc=(placedb.zl * 1.0 + placedb.zh * 1.0 - placedb.cell_depth * 1.0) / 2,
                                scale=(placedb.zh - placedb.zl) * 0.001,
                                size=placedb.num_movable_nodes)
            elif params.random_center_init_flag == 2:
                self.init_pos[placedb.num_nodes * 2 : placedb.num_nodes * 2 +
                            placedb.num_movable_nodes] = np.random.uniform(
        ...
        if placedb.num_filler_nodes:  # uniformly distribute filler cells in the layout
            if len(placedb.regions) > 0:
                ...
                # placedb.num_physical_nodes 到 placedb.num_nodes  就是填充物所在的下标
                ...
                self.init_pos[
                    placedb.num_nodes * 2 + placedb.num_physical_nodes : placedb.num_nodes * 3
                ] = np.random.uniform(
                    low=placedb.zl,
                    high=placedb.zh - placedb.node_size_z[-placedb.num_filler_nodes],
                    size=placedb.num_filler_nodes,
                )
        ...
        #self.pos_z = nn.ParameterList([nn.Parameter(torch.from_numpy(self.init_pos_z).to(self.device))]) # 3D modify
        ...
        # legality check
        self.op_collections.legality_check_op = self.build_legality_check(  # 3D modify 只修改了接口，没有修改最后legality_check.h文件里面的逻辑，因为z轴暂时不需要做合法化检查
            params, placedb, self.data_collections, self.device)
        # legalization
        if len(placedb.regions) > 0:
        ...
        self.op_collections.draw_place_op = self.build_draw_placement(
            params, placedb)
        self.op_collections.draw_place_op_2d = self.build_draw_placement_2d(
            params, placedb)
    ...
    def build_pin_pos(self, params, placedb, data_collections, device):
        ...
        return pin_pos.PinPos(
            ...
            pin_offset_z=data_collections.pin_offset_z,  # 3D modify
    ...
    def build_legalization(self, params, placedb, data_collections, device):
        ...
        ml = macro_legalize.MacroLegalize(
            node_size_x=data_collections.node_size_x, # 需修改
            node_size_y=data_collections.node_size_y, # 需修改
            ...
            num_movable_nodes=placedb.num_movable_nodes, # 需修改
        ...
        gl = legalize_alg(
            ...
            num_movable_nodes=placedb.num_movable_nodes,
            num_terminal_NIs=placedb.num_terminal_NIs, # 没问题 一般都是0
            num_filler_nodes=placedb.num_filler_nodes)  # 后续加filler时需要修改
        # for standard cell legalization
        ...
        def build_legalization_op(pos, node_size_x, node_size_y, num_terminals, num_movable_nodes, layer_num_fillers, row_height=None):
            logging.info("Start legalization")
            pos1 = ml(pos, pos, node_size_x, node_size_y, num_movable_nodes, layer_num_fillers, row_height)
            pos2 = gl(pos1, pos1, node_size_x, node_size_y, num_movable_nodes, layer_num_fillers, row_height)
            legal = self.op_collections.legality_check_op(pos2, node_size_x, node_size_y, num_terminals, num_movable_nodes,row_height)
            if not legal:
                logging.error("legality check failed in greedy legalization, " \
                    "return illegal results after greedy legalization.")
                return pos2
            pos3 = al(pos1, pos2, node_size_x, node_size_y, num_movable_nodes, layer_num_fillers, row_height)
            legal = self.op_collections.legality_check_op(pos3, node_size_x, node_size_y, num_terminals, num_movable_nodes,row_height)
            if not legal:
    ...
    def build_draw_placement_2d(self, params, placedb):
        """
        @brief plot placement
        @param params parameters
        @param placedb placement database
        """
        return draw_place.DrawPlace2D(placedb)
    ...
    def plot(self, params, placedb, iteration, pos, draw_mode = "3D", best=None):
        """
        @brief plot layout
        @param params parameters
        @param placedb placement database
        ...
        figname = "%s/plot/iter%s.pdf" % (path, '{:04}'.format(iteration))
        if best != None:
            figname = "%s/iter_best.pdf" % (path)
            logging.info("best iter is:%s"% ('{:04}'.format(iteration)))
        os.system("mkdir -p %s" % (os.path.dirname(figname)))
        if isinstance(pos, np.ndarray):
            pos = torch.from_numpy(pos)
        self.op_collections.draw_place_op(pos, figname, draw_mode)
        logging.info("plotting to %s takes %.3f seconds" %
                     (figname, time.time() - tt))
        
    def plot2d(self, params, node_size_x, node_size_y, iteration, pos, num_movable_nodes, num_filler_nodes, stage="gp"):
        """
        @brief plot layout
        @param params parameters
        @param placedb placement database
        ...
        tt = time.time()
        path = "%s/%s" % (params.result_dir, params.design_name())
        figname = "%s/tier/%s_tier%s.png" % (path, stage,'{:02}'.format(iteration))
        os.system("mkdir -p %s" % (os.path.dirname(figname)))
        if isinstance(pos, np.ndarray):
            pos = torch.from_numpy(pos)
        self.op_collections.draw_place_op_2d(pos, figname, node_size_x, node_size_y, num_movable_nodes, num_filler_nodes)
        logging.info("plotting to %s takes %.3f seconds" %
                     (figname, time.time() - tt))
    
    def plotTerminal(self, params, node_size_x, node_size_y, pos, num_movable_nodes, num_filler_nodes):
        """
        @brief plot layout
        @param params parameters
        @param placedb placement database
        ...
        tt = time.time()
        path = "%s/%s" % (params.result_dir, params.design_name())
        figname = "%s/tier/terminal.png" % (path)
        os.system("mkdir -p %s" % (os.path.dirname(figname)))
        if isinstance(pos, np.ndarray):
            pos = torch.from_numpy(pos)
        self.op_collections.draw_place_op_2d(pos, figname, node_size_x, node_size_y, num_movable_nodes, num_filler_nodes)
