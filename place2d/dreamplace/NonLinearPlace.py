...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    def __call__(self, params, placedb, legal):
        """
        ...
        if params.global_place_flag:
            ...
            for global_place_params in params.global_place_stages:
                ...
                if iteration == 0:
                    if params.gp_noise_ratio > 0.0:
                        ...
                        model.op_collections.noise_op(model.data_collections.pos[0], params.gp_noise_ratio) #加入噪音
                ...
                def one_descent_step(
                    ...
                    cur_metric = EvalMetrics.EvalMetrics(  #初始化Eval
                    ...
                    pos = model.data_collections.pos[0]  #随机的初始位置（用nn.Parameter做的参数初始化）

                    # move any out-of-bound cell back to placement region
                    self.op_collections.move_boundary_op(pos)   # 将超出的按x轴或y轴平移回来
                    ...
                    if model.update_mask is not None:
                        ...
                        optimizer.step()  #前向传播 
                        # 跳NesterovAcceleratedGradientOptimizer  step
                ...
                def check_plateau(x, window=10, threshold=0.001):
                    if len(x) < window:
                        return False
                    x = x[-window:]
                    #由于terminal，可能存在一个的情况，导致overflow一直为0
                    if np.mean(x) == 0.0:
                        return False
                ...
                with trange(model.Lgamma_iteration) as pbar:
                    # for Lgamma_step in tqdm(range(model.Lgamma_iteration)):  #线长近似模型中的平滑参数
                    for Lgamma_step in pbar:
                        Lgamma_metrics.append([])
                        Llambda_metrics = Lgamma_metrics[-1]
                        for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):   #目标函数中的线长与密度的参数
                            ...
                            for Lsub_step in range(model.Lsub_iteration):
                                ...
                                pbar.set_description("place2d iter%d" % Lgamma_step)
            ...
            # 输出GP后的线长和密度
            print("[INFO   ] Place3D - after GP HPWL: %.3f Overflow %.3f" % (best_metric[0].hpwl, best_metric[0].overflow))
        ...
        # plot placement
        if params.plot_flag:
            self.plot(params, placedb, iteration, self.pos[0].data.clone().cpu().numpy(), "GP")
            # 将模块大小还原再画一次
            # for i in range(0, len(placedb.node_size_x)):
            #     placedb.node_size_x[i] = placedb.node_size_x[i] * 2
        ...
        if params.legalize_flag:
            tt = time.time()
            onlyMacroLgFlag = 0
            # macroLgFlag 为true时单独合法化宏块
            if hasattr(params, "onlyMacroLgFlag"):
                onlyMacroLgFlag = params.onlyMacroLgFlag
            legalTemp = [True, True] # macro合法  cell合法
            self.pos[0].data.copy_(self.op_collections.legalize_op(self.pos[0],legalTemp,onlyMacroLgFlag))   # 合法化在此  跳转BasicPlace 416行
            logging.info("legalization takes %.3f seconds" % (time.time() - tt))
            cur_metric = EvalMetrics.EvalMetrics(iteration)
            all_metrics.append(cur_metric)
            cur_metric.evaluate(placedb, {"hpwl": self.op_collections.hpwl_op}, self.pos[0])
            # 输出合法化后的线长
            print("[INFO   ] Place3D - after Legalize HPWL: %.3f" % (cur_metric.hpwl))
            logging.info(cur_metric)
            iteration += 1
            # plot placement
            if params.plot_flag:
                self.plot(params, placedb, iteration, self.pos[0].data.clone().cpu().numpy(), "LG")
            
            if not legalTemp[0]: # 宏块不合法
                print(f"[ERROR  ] Place3D - layer {params.curLayer} legality check failed in macro legalization")
            else:
                print(f"[INFO   ] Place3D - layer {params.curLayer} legality check successful in macro legalization")
            if not legalTemp[1]: # cell不合法时才退出
                print(f"[ERROR  ] Place3D - layer {params.curLayer} legality check failed in greedy legalization")
            else:
                print(f"[INFO   ] Place3D - layer {params.curLayer} legality check successful in greedy legalization")
            legal[0] = legalTemp[0] # 将是否合法化传回去
            legal[1] = legalTemp[1] # 将是否合法化传回去
        ...
        if params.detailed_place_flag:
            ...
            cur_metric.evaluate(placedb, {"hpwl": self.op_collections.hpwl_op}, self.pos[0])
            # 输出DP后的线长
            print("[INFO   ] Place3D - after DP HPWL: %.3f" % (cur_metric.hpwl))
            logging.info(cur_metric)
            iteration += 1
            # plot placement
            if params.plot_flag:
                self.plot(params, placedb, iteration, self.pos[0].data.clone().cpu().numpy(), "DP")
