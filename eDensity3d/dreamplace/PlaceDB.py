...
class PlaceDB (object):
    ...
    def __init__(self):
        ...
        self.node_z = None # 3D modify, 1D array, cell position z
        ...
        self.node_size_z = None # 3D modify, cell的z轴depth  设置为 layerDepth
        self.layer = None # 3D modify 层数
        self.node2layer = None # 每个node所处于的层
        self.layer2nodes = None #  每层含有的node的下标
        # 存的都是张量
        self.layer_node_size_x = None # 存储每层的cell的长度的二维列表
        self.layer_node_size_y = None # 存储每层的cell的宽度的二维列表
        self.layer_gp_node_pos = None # 每层的GP后二维坐标
        self.layer_lg_node_pos = None # 每层的LG后二维坐标
        self.layer_num_movable_nodes = None # 每层可移动的cell的数量
        self.layer_num_terminals = None # 每次固定的块，只有第一层有
        self.layer_num_fillers = None #记录划分后每层的filler数量
        ...
        self.pin_offset_z = None # 3D modify, pin offset z 一般为z_max/4
        ...
        self.xh = None
        self.yh = None
        self.zl = None  # 3D modify
        self.zh = None  # 3D modify z轴的高度，一般为z_max

        self.row_height = None
        self.site_width = None
        self.cell_depth = None # 3D modify

        self.bin_size_x = None
        self.bin_size_y = None
        self.bin_size_z = None # 3D modify  z轴bin的深度
        self.num_bins_x = None
        self.num_bins_y = None
        self.num_bins_z = None # 3D modify  z轴bin的数量 
        ...
        self.dtype = None
        self.z_max = None # max of z
        self.zAlpha = 1.0 # z轴权重参数
    ...
    def area(self):
        ...
        """
        return self.width*self.height*(self.zh - self.zl)
    ...
    def initialize_from_rawdb(self, params):
        ...
        self.z_max = params.z_max if hasattr(params, "z_max") else 1000
        self.zAlpha = float(params.zAlpha) if hasattr(params, "zAlpha") else 1.0 #3D modify  z轴权重参数
        # self.cell_depth = float(self.z_max/self.layer) # 3D modify 
        # #单元高度设置成row_height  z轴高度设置成行高*层数
        self.cell_depth = params.layerDepth if hasattr(params, "layerDepth") else 500
        
        self.num_bins_z = params.num_bins_z if hasattr(params, "num_bins_z") else 8
        self.layer = params.layer if hasattr(params, "layer") else 2 #否则两层
        
        self.node_z = np.array(np.full(len(self.node_x),0),dtype=self.dtype) # 3D modify z的坐标默认为0
        self.node_size_x = np.array(pydb.node_size_x, dtype=self.dtype)
        self.node_size_y = np.array(pydb.node_size_y, dtype=self.dtype)
        self.node_size_z = np.array(np.full((len(pydb.node_size_x)), self.cell_depth),dtype=self.dtype) # 3D modify
        # 最后一个固定宏块高度设置为z_max
        if self.node_names[-1].decode() == "v1":
            self.node_size_z[-1] = self.z_max
            print(f"[INFO   ] v1 virtual node set size_z to {self.z_max}")
        # self.layers = np.array(np.full(len(self.node_x),0),dtype=self.dtype) # 3D modify 默认在0层
        ...
        self.pin_offset_z = np.array(np.full((len(pydb.pin_offset_x)), self.cell_depth / 2),dtype=self.dtype) # 3D modify
        ...
        self.zl = float(0)  # 3D modify
        
        self.row_height = float(pydb.row_height)
        
        self.zh = float(self.z_max) # 3D modify
        # z轴高度设置成行高*层数
        # self.zh = float(self.layer*self.cell_depth)
        
        self.site_width = float(pydb.site_width)
        self.num_movable_pins = pydb.num_movable_pins
        self.total_space_area = float(pydb.total_space_area)* (self.cell_depth) 
    ...
    def calc_num_filler_for_fence_region(self, region_id, node2fence_region_map, target_density):
        ...
        total_movable_node_area = np.sum(
            ...
            * self.node_size_z[:num_movable_nodes][fence_region_mask]
        )
...
die area = (x:%g, %g, y:%g, %g, z:%g, %g) %g
row height = %g, cell depth = %g, site width = %g
""" % (
                self.num_physical_nodes, self.num_terminals, self.num_terminal_NIs, self.num_movable_nodes, len(self.net_names),
                self.xl, self.xh, self.yl, self.yh, self.zl, self.zh, self.area,
                self.row_height, self.cell_depth, self.site_width
                )
        ...
        # set total cell area
        self.total_movable_node_area = float(np.sum(self.node_size_x[:self.num_movable_nodes]*self.node_size_y[:self.num_movable_nodes]*self.node_size_z[:self.num_movable_nodes])) # 3D modify 实际上已经是体积 volume
        ...
        self.total_fixed_node_area = float(np.sum(
                np.maximum(
                    ...
                    0.0) * np.maximum(
                        ...
                        0.0) * np.maximum(
                        np.minimum(self.node_z[self.num_movable_nodes:self.num_physical_nodes - self.num_terminal_NIs] + self.node_size_z[self.num_movable_nodes:self.num_physical_nodes - self.num_terminal_NIs], self.zh)
                        - np.maximum(self.node_z[self.num_movable_nodes:self.num_physical_nodes - self.num_terminal_NIs], self.zl),
                        0.0)
                ))
        ...
        target_density = min(self.total_movable_node_area / self.total_space_area, 1.0)  # 计算理论最小的可移动面积比总面积
        ...
        if params.enable_fillers:  # area都被视作volum
            ...
            if len(self.regions) > 0:
                ...
                node_size_order_y = np.argsort(self.node_size_y[: self.num_movable_nodes])
                ...
                filler_size_y = np.mean(self.node_size_y[node_size_order_y[range_lb:range_ub]])
                
                filler_size_z = self.cell_depth # 3D modify
                placeable_area = max(self.area - self.total_fixed_node_area, self.total_space_area) # self.area, self.total_fixed_node_area ,self.total_space_area 是体积的
                ...
                filler_area = filler_size_x * filler_size_y * filler_size_z #area即是体积
                if filler_area == 0: 
                    self.num_filler_nodes = 0
                else:
                    ...
                    self.node_size_z = np.concatenate( # 3D modify
                        [
                            self.node_size_z,
                            np.full(self.num_filler_nodes, fill_value=filler_size_z, dtype=self.node_size_z.dtype),
                        ]
                    )
                content += "total_filler_node_area = %g, #fillers = %d, filler sizes = %gx%gx%g\n" % (
                    self.total_filler_node_area,
                    self.num_filler_nodes,
                    filler_size_x,
                    filler_size_y,
            ...
            self.total_filler_node_area = 0
            self.num_filler_nodes = 0
            filler_size_x, filler_size_y, filler_size_z = 0, 0, 0
            if len(self.regions) > 0:
        ...
        self.bin_size_z = (self.zh - self.zl) / self.num_bins_z # 3D modify

        content += "num_bins = %dx%dx%d, bin sizes = %gx%gx%g\n" % (self.num_bins_x, self.num_bins_y, self.num_bins_z, self.bin_size_x, self.bin_size_y, self.bin_size_z)
    ...
    def write(self, params, filename, sol_file_format=None):
        ...
        """
        tt = time.time()
        logging.info("placeDB.write-- writing to %s" % (filename))
    ...
    def write_pl(self, params, pl_file, node_x, node_y):
        ...
        """
        tt = time.time()
        # logging.info("placeDB.write_pl-- writing to %s" % (pl_file))
        content = "UCLA pl 1.0\n"
    ...
    def apply(self, params, node_x, node_y, node_z):
        ...
        self.node_z[:self.num_movable_nodes] = node_z[:self.num_movable_nodes] # 3D modify

        # unscale locations
        node_x, node_y = self.unscale_pl(params.shift_factor, params.scale_factor) # 未改
